...
public class ConnectionHandler implements Runnable {
	...
	//public static final int SOCKET_CONNECTION_TIMEOUT = 5*1000;
	public static final int SOCKET_CONNECTION_TIMEOUT = 3*1000;
	...
	private void accept() throws IOException, SocketTimeoutException {
		...
		if(socket == null){
			logger.error("socket accept is NULL!");
			return;
		}
		...
		//System.out.println(this.address.getPort() + ":" + port);
		...
		try {
			...
			logger.error("Invalid handshake from {}: {}",
				this.socketRepr(socket), pe.getMessage());
			try { socket.close(); } catch (IOException e) { }
		} catch (IOException ioe) {
			logger.error("An error occured while reading an incoming " +
	...
	public boolean connect(SharingPeer peer) {
		...
		try {
			socket.connect(address, SOCKET_CONNECTION_TIMEOUT);
		} catch (IOException ioe) {
