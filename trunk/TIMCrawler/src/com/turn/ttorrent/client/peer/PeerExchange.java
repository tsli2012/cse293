...
class PeerExchange {
	...
	//private static final int KEEP_ALIVE_FOR_MINUTES = 60;
	...
	public PeerExchange(SharingPeer peer, SharedTorrent torrent,
		...
		//logger.error("PeerExchange start");
		...
		//logger.debug("PeerExchange unbind");
...
//		BitSet pieces = this.torrent.getCompletedPieces();
	...
	public void terminate() {
		...
		logger.error("Terminated peer exchange with {}.", this.peer);
	}
		...
		public void run() {
			try {
				...
				while (!stop) {
					...
					is.readFully(buffer.array(), 4, len);

					try {
						Message message = Message.parse(buffer, torrent);
						logger.debug("Received {} from {}", message, peer);
			...
			/*catch (EOFException ex) {
				...
				//logger.info("*** Could not read message from" + peer + "***");
				logger.warn("Could not read message from {}: {}",
					new Object[] { peer, ioe.getMessage(), ioe, ioe instanceof EOFException});
				//System.err.println(ioe instanceof SocketTimeoutException);
				//ioe.printStackTrace();
				peer.unbind(true);
			}
		}
	...
	private class OutgoingThread extends Thread {
		...
		public void run() {
			try {
				...
				logger.warn("Could not send message to {}: {}",
