...
public class SharingPeer extends Peer implements MessageListener {
	...
	public SharingPeer(String ip, int port, ByteBuffer peerId,
		...
		this.listeners = Collections.newSetFromMap(new ConcurrentHashMap<PeerActivityListener,Boolean>());
	 ...
	 */
	public boolean unbindStatus = false;
	
	public void unbind(boolean force) {
		unbindStatus = false;
		logger.debug("at unbind");
		if (!force) {
		...
		synchronized (this.exchangeLock) {
			logger.debug("at unbind: into unbiexchangeLocknd");
			
			if (this.exchange != null) {
				logger.debug("at unbind: exchange");
				if (force) {
	...
	public synchronized void handleMessage(Message msg) {
		switch (msg.getType()) {
			...
			case BITFIELD:
				...
				this.fireBitfieldAvailabity(); //tsli
				break;
			case REQUEST:
				...
				// tsli: not need to handle REQUEST?
				/*if (this.isChoking() ||
			...
			case PIECE:
				...
				//boolean shouldIgnorePiece = false;
				boolean shouldIgnorePiece = true;
				...
				/*else {
					...
					try {
						...
						if (piece.getOffset() + piece.getBlock().capacity()
								== p.size()) {
							p.validate();
							//this.firePieceCompleted(p); tsli
							this.requestedPiece = null;
							//this.firePeerReady(); tsli
						} else {
							//this.requestNextBlocks(); tsli
						}
					} catch (IOException ioe) {
						this.fireIOException(ioe);
						break;
